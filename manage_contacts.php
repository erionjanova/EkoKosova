...
if(isset($_GET['delete'])){
    ...
    $deleteQuery = $conn->prepare("DELETE FROM contacts WHERE id=?");
    $deleteQuery->execute([$id]);

    header("Location: manage_contacts.php"); // rifreskon faqen
    exit;
}

    // Merr të gjitha kontaktimet nga databaza (të gjitha fushat)
    $contactsQuery = $conn->query("SELECT id, full_name, email, subject, message, created_at FROM contacts ORDER BY created_at ASC");
...
<title>Menaxho Kontaktet | Dashboard</title>
...
<main>
<h1 style="text-align:center">📩 Menaxho Kontaktet</h1>
...
<tr>
    ...
    <td class="message"><?= htmlspecialchars($c['message']) ?></td>
    <td><?= date('d.m.Y H:i', strtotime($c['created_at'])) ?></td>
    <td>
        <a href="edit_contacts.php?id=<?= $c['id'] ?>" class="btn_edit">✏️</a>
        <a href="#" class="btn delete" data-id="<?= $c['id'] ?>">🗑️</a>
    </td>
</tr>
<?php endforeach; ?>
</table>
...
<div id="deleteModal" class="modal">
    <div class="modal-content">
        ...
        <p>Ky kontakt do të fshihet përgjithmonë.</p>
        <div style="display:flex; justify-content:center; gap:10px;">
            <a href="#" class="btn delete" id="confirmDelete">Fshij</a>
